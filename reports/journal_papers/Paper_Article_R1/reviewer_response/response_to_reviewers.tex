...
\begin{document}
	...
	\color{blue}{\textbf{Response:} We appreciate your constructive comment.
		Actually some papers of Young-Jin Cha (U Manitoba) and others on CNN in Civil engineering and rotating machinery domains were included in the updated version.
		...
		Therefore, in the literature we only included Deep learning/ Neural Network-based guided wave-based damage/delamination detection, as our main contribution is regarding the use of deep learning methods on guided full wavefield based delamination detection.
	\\ \\
...
%	\\ \\
	...
	However, in the FCN-DenseNet model, authors first apply the Batch Normalization followed by the activation function (ReLU) then they applied the convolution. 
	...
	Accordingly, the threshold must be set to decide whether the output belongs to the class undamaged represented by \(0\) value or damaged represented by \(1\) value. 
	...
	\\ \\
	\color{blue}{\textbf{Response:} Yes, others meant et al., we checked the reference and updated to et al.}
	\\ \\
