...
As shown in Fig.~\ref{fig:points_metrics} when the value of $N_p$ increases we get a better recovery of both the full HR frame and the at the exact delamination region.
However, for lower values of $N_P$ such $N_p=1024$ the PSNR and Pearson CC values for recovered HR frame was poor indicating that the conventional CS method is not efficient below the Nyquist sampling rate.
...
Figure~\ref{fig:frame110_comparison} shows the recovery of the reference frame presented in Fig.~\ref{fig:frame110_CS1024} at $N_f=110$ with CS method and DLSR models.
...
Both DLSR models recovered the HR frame with $N_p=1024$ points, which is \(19.2\%\) of Nyquist sampling rate.
...
Furthermore, Fig.~\ref{fig:frame110del_comparison} presents the recovery of the HR frame $N_f=110$ at delamination region with respects to Fig.~\ref{fig:frame110_comparison}.
Figure~\ref{fig:frame110delam_ref} shows the reference region that we are looking to recover, which shows reflected waves from damage.
Figures~\ref{fig:frame110delam_CS1024} and~\ref{fig:frame110delam_CS3000} show a poor reconstruction of the reflected waves at $N_p=1024$ and $N_p=3000$, respectively,using CS method.
Figure~\ref{fig:frame110delam_CS4000} shows the recovery of the reflected frames at $N_p=4000$ using CS method, in which the reflected waves are recognisable.
Figures~\ref{fig:frame110delam_Abdalraheem} and~\ref{fig:frame110delam_Saeed} shows the recovery of the reflected waves using DLSR models, respectively.
The recovered reflected waves from damage using DLSR models is clear and can be easily recognisable, which indicates that the DLSR approach can be more efficient than the conventional CS method. 
...
Table~\ref{tab:csv_results} presents a detailed comparison of the quality metrics for CS methods and DLSR models for various number of points $N_p$ and the corresponding ratios CR calculated for the frame no $N_p=110$.
