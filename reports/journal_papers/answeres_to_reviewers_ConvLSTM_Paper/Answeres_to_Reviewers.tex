...
\begin{document}
	...
	\begin{enumerate}
		...
		delamination case, which leads to less accurate damage imaging.
		...
		\textcolor{Cyan}{
			...
			Unfortunately, we cannot use other datasets of this type because such datasets are not available.
		}
	...
	\textcolor{Cyan}{
		\textbf{Response:}
	Thank you for your suggestion. However, we think that the maths notation used for description
	of the RNNs, LSTMs, ConvLSTM is standard and widely used in the literature.
...
\begin{enumerate}
	...
	\textcolor{Cyan}{
		...
		The other option is to used air-coupled transducer attached to motorized arm for automatic sensing.
		Therefore, if SLDV is not available such techniques can be applied for 
		acquiring the Lamb wave-based data.  
	}
...
\textcolor{Cyan}{
	\textbf{Response:} \\
	Thank you for pointing this out. 
	The lower number of frames in the experimental case with one delamination is because this experiment was conducted few years back and we wanted to utilize that data. Now, we usually acquire 512 frames so that the resulting file is not excessively large and easy for post processing by eg. 3D FFT. In general, the most important aspect is to acquire  long enough signals so that guided waves propagate at least to the most distant edge of the structure.
	}
...
\textcolor{Cyan}
{
	...
	\begin{table}[ht!]
		...
		\begin{tabular}{lccc}
			...
			\bottomrule
		\end{tabular}
	\caption{IoU metric of the three numerical cases with respect to model 
	FCN-DenseNet~[\textcolor{red}{47}] and GCN~[\textcolor{red}{47}]}
	\label{num_cases}
	\end{table}
...
show the damage maps predicted by GCN~[\textcolor{red}{47}] model.
...
may refer to~[\textcolor{red}{46, 47}].
	...
	\begin{figure}[ht!]
		...
		\end{subfigure}
		\caption{Predictions of FCN-DenseNet~[\textcolor{red}{47}] and 
		GCN~[\textcolor{red}{47}] with respect to the three numerical test 
		cases.}
...
\begin{enumerate}
	...
	\textcolor{Cyan}{
		\textbf{Response:}
		Thank you for your comment. We understand your concern, and we agree that the method posses some limitations. The ConvLSTM model was trained only on one type of material (CFRP) and is most suitable for damage imaging in structures made of such materials. However, the dataset can be extended to other materials e.g. GFRP and the ConvLSTM model can be retrained. Then the ConvLSTM model should be able to generalize. The same applies for different types of defects.\\
		The models are fed by the full wavefield data (animations) which represents out-of-plane displacements or velocities for direct measurements by SLDV on the surface of the inspected structure. Due to this fact, all dispersion characteristics are already embedded into the data. It is irrelevant if there are more modes propagating (e.g. SH0, A1, S1) since the ConvLSTM model performs feature extraction without the need for handcrafting features.
	}
	...
	\textcolor{Cyan}{
		\textbf{Response:}
		Thank you for your constructive comment.\\
		Since sigmoid function is used for predicting final outputs, it is the best to apply fixed threshold at 0.5. Moreover, this guarantee consistent comparison of results obtained by various deep learning models (in our case Model-I and Model-II).\\
		For other signal processing methods such as adaptive wavenumber filtering, adjustable threshold could perform better.
	}
