...
\begin{sloppypar}
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	In this work, we developed two end-to-end deep learning models utilising full wavefield frames of Lamb wave propagation for delamination identification in CFRP materials\DIFdelbegin \DIFdel{, as presented in Fig.
	~\ref{fig:proposed_models}.
	}\DIFdelend \DIFaddbegin \DIFadd{.
	}\DIFaddend The developed models have a scheme of many-to-one sequence prediction, which takes \(m\) number of frames representing the full wavefield propagation through time and their interaction with the delamination to extract the damage features and finally predict the delamination location, shape, and size in a single output image.
...
%DIFDELCMD < 			\centering
%DIFDELCMD < 			\includegraphics[width=.2\textheight]{figure5a.png}
%DIFDELCMD < 			%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Model-}%DIFDELCMD < \RNum{1}%%%
} %DIFAUXCMD
%DIF <  : Convolutional LSTM model.
			%DIFDELCMD < \label{fig:convlstm_model}
%DIFDELCMD < 		\end{subfigure}
%DIFDELCMD < 		\hfill
%DIFDELCMD < 		\begin{subfigure}[b]{0.49\textwidth}
%DIFDELCMD < 			\centering
%DIFDELCMD < 			\includegraphics[width=.2\textheight]{figure5b.png}
%DIFDELCMD < 			%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Model-}%DIFDELCMD < \RNum{2}%%%
} %DIFAUXCMD
%DIF <  : Time distributed AE model.
			%DIFDELCMD < \label{fig:AE_convlstm}
%DIFDELCMD < 		\end{subfigure}
%DIFDELCMD < 		%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{The architecture of the proposed deep learning models.}}
		%DIFAUXCMD
%DIFDELCMD < \label{fig:proposed_models}
%DIFDELCMD < 	\end{figure} 
%DIFDELCMD < 	%%%
\DIFdelend 

	%DIF < %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	The first proposed model \DIFaddbegin \DIFadd{(Model-I)}\DIFaddend , presented in Fig.~\ref{fig:convlstm_model} takes \(64\) frames as input, and it consists of three ConvLSTM layers that can process time series and computer vision tasks.
...
%DIF > 		\begin{subfigure}[b]{0.49\textwidth}
			\centering
			\includegraphics[width=.35\textwidth]{figure5a.png}
			\caption{\DIFaddFL{Model-}\RNum{1}\DIFaddFL{.}} %DIF >  : Convolutional LSTM model.
...
\DIFaddend 
	...
	In the second implemented model \DIFaddbegin \DIFadd{(Model-II) }\DIFaddend presented in Fig.~\ref{fig:AE_convlstm}, we applied an autoencoder technique (AE)\DIFaddbegin \DIFadd{, }\DIFaddend which is well-known for extracting spatial features.
	\DIFaddbegin 

	\begin{figure} [\DIFaddFL{ht!}]
%DIF > 		\begin{subfigure}[b]{0.49\textwidth}
			\centering
			\includegraphics[width=1\textwidth]{Model_II.png}
			\caption{\DIFaddFL{Model-}\RNum{2}\DIFaddFL{.}} %DIF >  : Time distributed AE model.
			\label{fig:AE_convlstm}
%DIF > 		\end{subfigure}
%DIF > 		\caption{The architecture of the proposed deep learning models.}
%DIF > 		\label{fig:proposed_models}
	\end{figure} 
	...
	\DIFdelbegin \DIFdel{In general, an }\DIFdelend \DIFaddbegin \DIFadd{An }\DIFaddend AE consists of three parts: the encoder, the bottleneck, and the decoder.
	...
	\DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{Every level at the }\DIFaddend decoder part consists of two 2D convolutional operations followed by batch normalization and dropout, and an upsampling is applied at the end of each decoder level to retrieve the dimensions of its inputs.
