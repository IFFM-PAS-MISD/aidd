...
\begin{document}
\chapter[\DIFadd{ML for SHM: Review}]{\DIFadd{Machine Learning for SHM:} Literature Review}
...
Generally, Guided-Wave Based SHM systems can be built upon processing of signals registered by piezoelectric transducers (PZTs) or Scanning Laser \DIFdel{Dopper} \DIFadd{Doppler} Vibrometry (SLDV).
...
\paragraph{Scanning Laser \DIFdel{Dopper} \DIFadd{Doppler} Vibrometer}
...
\section[Introduction to AI, ML and deep learning]{Introduction to AI, machine learning, \DIFdel{and} \DIFadd{\\and} deep learning}
...
Historically, \DIFdel{Artificial Intelligence} \DIFadd{AI} was \DIFdel{in} introduced in 1956 at the Dartmouth summer conference by John McCarthy.
For many years after, AI has been in what so-called AI winter due to the lack of \DIFdel{powerful}  computational \DIFdel{computers and} \DIFadd{power.
...
\DIFdel{In which a} 
GPU \DIFadd{computational power} exceeds the traditional \DIFdel{CPUs,} \DIFadd{Central Processing Units (CPUs),} due to the high capability of parallel computing, which makes it more efficient in running algorithms for large \DIFdel{data~\cite{}.} \DIFadd{data.}
The second reason is the tremendous Data available \DIFdel{in the current time,} \DIFadd{nowadays,} which can \DIFdel{a remarkable development} \DIFadd{remarkably improve} the learning process of an AI system, since, its effectiveness depends on learning from its environment. 
...
\todo[inline]{Avoid vertical lines in tables. It just looks better. }

\verb+\renewcommand{\arraystretch}{1.1}+
\todo[inline]{gives more air between lines}
...
The conventional way of software engineering is through creating rules by human and combine them with data to create a solution to \DIFadd{a} problem.
Alternatively, when it comes to machine learning, it utilises data and answers to learn the rules behind \DIFdel{a} \DIFadd{the} problem~\cite{franoischollet2017learning}.
...
There are different methods \DIFadd{which} can be implemented when performing machine learning. 
Generally, those methods are grouped into three traditional approaches: Supervised learning, Unsupervised \DIFdel{learning and} \DIFadd{learning,} Reinforcement learning, and \DIFdel{a} Transfer learning approach.
...
\end{figure}
\todo[inline]{Figure \ref{fig:Machine_learning_approaches} is very cluttered. I would remove all boxes and try to make it in a similar style as Fig.~\ref{fig:ReinforcementLearning} }

\paragraph{Supervised learning}
s the task of \DIFdel{leaning} \DIFadd{learning} a machine the inference rules from the training data, and how to map inputs with outputs.
The training data is a collection of variables together with its labels e.g. a set of civil images of structures that are labelled as cracked or \DIFdel{rebar exposure.} \DIFadd{undamaged.}
...
\DIFdel{Machine learning techniques are suffering from a lack of the ability to process raw data immediately. 
...
\todo[inline]{Mathematical functions such as cosine or hyperbolic tangent must be written in upright notation. LaTeX has embedded such functions:}
\verb+\tanh+
\todo[inline]{Partial derivatives \(\partial\) are typeset by:}
\verb+\partial+
...
Fig.~\ref{fig:GD} illustrates the concept of GD, in which weights \DIFdel{\(\{w_0,w_1,...,w_n\}\)are} \DIFadd{\(\{w_0,w_1,...,w_n\}\) are} initially assigned \DIFdel{at random.} \DIFadd{randomly.}
...
\paragraph{\DIFdel{ConvNets}\DIFadd{Convolutional Neural Network}}
...
The importance of SHM systems originated from their ability to monitor the \DIFdel{structure's} condition \DIFadd{of structures} in \DIFadd{a} real-time.
...
Therefore, introducing machine learning methods to the feature extraction process became \DIFdel{necessary, hence} \DIFadd{necessary.
Hence} machine learning methods have the capability to generalise and learn new features by themselves which improves \DIFdel{there} \DIFadd{their} functionality in damage estimation.


\DIFdel{In~\cite{Worden2007}authors}


\DIFadd{In~\cite{Worden2007} authors} have proposed several axioms related to SHM systems implemented using machine learning methods. 
...
Therefore, introducing machine learning methods to the feature extraction process became necessary, hence machine learning methods have the capability to generalise and learn new features by themselves which improves \DIFdel{there} \DIFadd{their} functionality in damage estimation.
...
Authors in ~\cite{Das2010} presented a method for estimating several types of defects (delamination, saw cut, notches and drilled \DIFdel{holes)in} \DIFadd{holes) in} composite material. For this purpose, a collection of PZT transducers were attached to the surface of the structure to \DIFdel{generating} \DIFadd{generate} and \DIFdel{registering} \DIFadd{register} Lamb waves propagation. 
Accordingly, a time-frequency domain were utilised to extract features relates to \DIFdel{the} defects from the registered response. Those extracted features were fed to one-class SVM, which performs classification and damage estimation. 
Moreover, in~\cite{Dib2018}, authors proposed a novelty classifier based on \DIFdel{one class} \DIFadd{one-class} SVM for detecting damage. The method was conducted by extracting data from damage impact on glass-fibre composite plate, then evaluating the performance on the classifier. To extract the necessary features \DIFdel{form} \DIFadd{from} the propagated wave, the registered signal was segmented into L time bins, then, the Fourier transform was applied on each time bin.
Accordingly, the features vector was constructed from the signal phase and its amplitude for each segmented time bin.

\DIFdel{Authors in~\cite{Vitola2016}}

\DIFadd{Vitola et al.~\cite{Vitola2016}} developed a damage detection and classification methodology that was examined on aluminium plates.
\DIFdel{accordingly, a group}
\DIFadd{An array} of PZT transducers \DIFadd{was} placed on the plate surface to sense wave propagation in the structure.
...
PCA means to analyse a large amount of \DIFdel{information,} \DIFadd{information} by finding the principal \DIFdel{components, however,} \DIFadd{components.
...
\DIFdel{Author in~\cite{Godin2004}} 

\DIFadd{Godin et al.~\cite{Godin2004}} applied Acoustic Emission signals (AE) in their approach, which happen due to a sudden release of stored energy when damage occurs.
...
Accordingly, the clustered labelled data \DIFdel{will be} \DIFadd{is} fed into a KNN supervised classifier.
A trained classifier \DIFdel{will be} \DIFadd{is} able to classify new coming data accordingly.
Regarding the unsupervised classification, Kohonen classifier was utilised~\cite{58325}, which is a self-organising map (SOM) which is a neural network \DIFdel{consists} \DIFadd{consisting} of neurons as processing units. 
...
For this purpose, the K-means algorithm with the genetic algorithm were utilised. PCA was used to reduce the data \DIFdel{dimensionality, next,} \DIFadd{dimensionality.
...
\DIFdel{Accordingly, parameters}
\DIFadd{Parameters} of the AE signals such as peak amplitude, frequency, rise time, energy and the duration were estimated for each cluster and utilised as discriminative features. 
...
For this purpose, authors attached a combination of \DIFdel{(PZT)} \DIFadd{PZT} transducers on the panel used to generate and receive Lamb waves that propagate through the structure. 
During their propagation through the \DIFdel{structure will} \DIFadd{structure, Lamb waves} encounter defects, which affects their propagation response. 
...
Full wavefield scanning using \DIFdel{LSDV} \DIFadd{SLDV} is a time consuming, however, by simply reducing the number of scanning points will result in low-quality images. 
\DIFdel{Accordingly, authors} 
\DIFadd{Authors} in~\cite{esfandabadideep} proposed a compressive Sensing technique using ConvNets to enhance the resolution for images captured by \DIFdel{LSDV} \DIFadd{SLDV} while decreasing the number of measurement scan points down to \(10\%\) of the number of the full gird scanning points. 
...
Furthermore, authors \DIFdel{in~\cite{Melville2018}proposed} \DIFadd{in~\cite{Melville2018} proposed} a technique for damage detection in thin metal plates (aluminum and steel), using full wavefield data scanned by \DIFdel{LSDV.} \DIFadd{SLDV.} Using this data to train a deep neural network of \DIFdel{4-hidden} \DIFadd{4 hidden} layers including \DIFdel{2-} \DIFadd{2} convolutional layers for features extraction and \DIFdel{2-} \DIFadd{2} fully connected layers. Their results show good results when compared with traditional machine learning SVM.
Moreover, the author \DIFdel{in~\cite{Melville2017}introduced} \DIFadd{in~\cite{Melville2017} introduced} a method for detecting damage in structures based on the k-means algorithm. The method is known as "dictionary learning" which uses full wavefield data collected from thin metal plates. 
...
\todo[inline]{Finish with the summary. Thesis description is not appropriate here. Please remove it from the chapter.}
\section{Thesis description}

\DIFdel{The aim of this thesis is to develop an Artificial Intelligence (AI) driven diagnostic system for delamination identification in composite laminates such as carbon fibre reinforced polymers (CFRP).
...
\DIFdel{will be focused on feasibility studies of machine learning approaches for elastic wave propagation analysis. 
...
\DIFdel{ modern end-to-end deep learning approach is proposed in opposite to traditional pipeline approach with intermediate components (hand-crafted feature extraction). 
