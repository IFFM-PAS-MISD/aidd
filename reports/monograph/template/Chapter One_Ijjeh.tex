...
\begin{itemize}
	\item Fibre reinforced composite materials that consist of three parts: the fibres as the discontinuous phase, the matrix as the continuous phase, in addition to the fine inter-phase region known as the interface~\cite{Cantwell1991}.
...
Generally, composites material defects can happen due to matrix cracking, fibre breakage, fibre-matrix debonding and delamination between layers, which frequently happen below the top surfaces and are barely visible~\cite{Cai2012}. 
...
Defects can only be discovered by analysing the responses of the structure, 
obtained by sensors, before and after it happens, hence, we cannot expect to have �damage sensors�.
Detecting the damage is achieved by processing and comparing the signals received from the sensors ahead and following damage occurrence~\cite{s18041094}. 
Then one can attempt to classify the parameters, which are sensitive to insignificant  damage and that can be distinguished from the response to natural and environmental disturbances~\cite{s18041094}. 
...
Designing a robust SHM system requires knowledge in various scientific fields e.g. electrical engineering and mechanical engineering, as well as in mathematics, computer science, and physics~\cite{Willberg2013}.
...
A common technique is the use of Lamb waves, which propagate 
...
Among them are the dispersive nature of Lamb wave propagating modes that can turn into each other in the presence of defects and other changes in the mechanical 
...
SLDV employs Doppler frequency shift principle to estimate the velocity of a moving object in which the amount of the shifted frequency depends on the velocity of the moving object~\cite{Stanbridge1999}. 
...
A 3D vibrometer measures a location with three independent beams that hit the target from three different directions, which yields a measurement of the complete out-of-plane and in-plane velocity of the target.
...
In general, the process of extracting features of the defects occurred in structures can be achieved in different domains: frequency domain, time domain, time-frequency domain, impedance domain, and modal analysis domain~\cite{Khan2019}.
