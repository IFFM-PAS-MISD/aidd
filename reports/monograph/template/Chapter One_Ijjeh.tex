...
Generally, those methods are grouped into three traditional approaches: Supervised learning, Unsupervised learning, Reinforcement learning, and Transfer learning approach.
...
Structures wether are civil (e.g. buildings, bridges, etc)or aeronautic (e.g. in composite materials) are ageing in time,  and are exposed to have several types of damage. 
...
To avoid these consequences SHM techniques could be applied.  
SHM techniques can detect any possible change that occurs at 
a structure that could decay the performance of the whole system, at the 
earliest possible time so that an action can be taken to reduce the downtime,  operational costs and maintenance costs, consequently reducing the risk of catastrophic failure, injury, or even loss of life.


There are several SMH approaches for detecting and localising damage within composite structures. A well-Known approach is using guided Lamb waves. 
Guided-Wave Based SHM systems can be built upon processing of signals registered by piezoelectric transducers (PZTs) or Scanning Laser Doppler Vibrometry (SLDV). 
Several authors have studied and examined Guided Lamb waves in composite materials to detect and localise the damage using signal processing techniques. 
Those traditional techniques are complex and involve a huge numerical analysis, signal processing, and the damage features are difficult to be extracted manually. 
Accordingly, new approaches that involve Machine and Deep Learning techniques are utilised. 
By doing so, the process of damage features extracting became more convenient and easier since the machine is responsible for learning the new features and accordingly detects and localising the damage. The advantage of this approach is the scalability. 
Which means that the performance of the machine learning system increases with the size of data.
