...
In deep learning, supervised learning is the traditional approach for learning~\cite{Lecun2015}. 
In supervised learning, the neural network builds its knowledge from the given labelled dataset, where the ground truth output is known previously.
By updating the network parameters and weights, the network learns to find the desired output by itself.
Initially, the network does a comparison between the calculated output (predicted) and the ground truth output (target).
...
Different artificial neural networks (ANN) were implemented in deep learning such as: Convolution Neural Network (CNN), Deep Belief networks (DBN) and Recurrent Neural network (RNN).
