...
Fig shows the three different types of Machine learning approaches.
\begin{figure} [h!]
	\begin{center}
		\centering
...
There are several non-linear functions used in artificial neural networks such as the Rectified Linear unit (Relu) which is used commonly, shown in Equation~\ref{Eq:relu}. Other non-linear functions are the Sigmoid logistic function as shown in Equation~\ref{sigmoid} and hyperbolic tangent function tanh as shown in Equation~\ref{tanh} ~\cite{Lecun2015}, where \(z\) is the summation of weights \(\{w_0,w_1,...,w_n \}\) multiplied by input variables (from previous layer) \(\{x_0,x_1,_...,x_n\}\) and a bias \(b\) as shown in Equation~\ref{z}.
...
Different artificial neural networks (ANN) were implemented in deep learning like Deep Neural Network (DNN), Convolution Neural Network (CNN), Deep Belief networks (DBN) and Recurrent Neural network (RNN).
