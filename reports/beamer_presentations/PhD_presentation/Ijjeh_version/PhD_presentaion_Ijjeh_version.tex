...
\begin{document}
	...
	\note{
		...
		Supervised by Professor Pawl Kudela.		
...
%					\hspace{5pt}
	...
	\end{frame}
	\note{
		Compressive sensing is a signal processing technique for efficiently acquiring and reconstructing a signal, by finding solutions to underdetermined (there are fewer equations than unknowns) linear systems. 
				
		There are two conditions under which recovery is possible.
		\begin{itemize}
			\item Sparsity, which requires the signal to be sparse in some domain many coefficients are close or equal to zero.

			\item Incoherence, which is applied through the isometric property, which is sufficient for sparse signals.
		\end{itemize}
		...
		In particular, in this paper we employed SPGL1 algorithm (A solver for large-scale sparse reconstruction) and Fourier basis.
