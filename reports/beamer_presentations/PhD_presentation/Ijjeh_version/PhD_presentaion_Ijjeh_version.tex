...
%					\hspace{5pt}
	...
	\end{frame}
	\note{
		The theory of compressed sensing (CS) states that natural signals, such as sounds and images, can be recovered using considerably fewer samples or measurements than standard methods.
				
		CS relies on two principles:
		\begin{itemize}
			\item Sparsity: which relates to the signal of interest.
			\item Incoherence: which relates to the sensing modality
		\end{itemize}		
		
		In particular, in this work SPGL1 algorithm (A solver for large-scale sparse reconstruction) was utilised and Fourier basis for \(\psi\).
