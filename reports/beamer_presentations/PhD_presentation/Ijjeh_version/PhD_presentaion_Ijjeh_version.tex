...
\begin{frame}{Advantages \& applications}
	\begin{minipage}[c]{.45\textwidth}
		...
		\begin{itemize}
			\only<2->{\item \textcolor{PineGreen}{High impact damage resistance}}
			\only<3->{\item \textcolor{PineGreen}{Light weight}}
			\only<4->{\item \textcolor{PineGreen}{High strength-to-weight ratio}}
			\only<5->{\item \textcolor{PineGreen}{Low density}}
			\only<6->{\item \textcolor{PineGreen}{resistance to fatigue and corrosion}}			
		\end{itemize}
	\end{minipage}
	\hfill
	\only<7->{\begin{minipage}[c]{.45\textwidth}
		Some applications of Composite laminates:
		\begin{itemize}
			\item \textcolor{PineGreen}{Aerospace structures}
			\item \textcolor{PineGreen}{Automotive}
			\item \textcolor{PineGreen}{Wind turbines}
			\item \textcolor{PineGreen}{Pipes and tanks}
		\end{itemize}
	\end{minipage}}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{frame}{Non Destructive Evaluations (NDEs)}
	...
	\end{minipage}}
	\only<3->{\begin{minipage}{0.32\textwidth}
			\begin{itemize}
			...
			\end{itemize}
			\only<4->{\textcolor{BrickRed}{Local NDEs are not practical and do not fit for SHM}}
	\end{minipage}}
...
\begin{frame}{Lamb waves modes}
	...
	\begin{columns}[T]
		\begin{column}{0.3\textwidth}
			\centering
			\begin{figure}
				\subfloat{\animategraphics[autoplay,loop,width=1\textwidth]{10}{figures/gif_figs/S0_mode/S0_mode-}{0}{67}}
		...
		\begin{column}{0.3\textwidth}
			\centering
			\begin{figure}
				\subfloat{\animategraphics[autoplay,loop,width=1\textwidth]{10}{figures/gif_figs/A0_mode/A0_mode-}{0}{67}}
		...
		\only<2->{
			\begin{column}{0.37\textwidth}
				\begin{itemize}
					\only<2->{\item \textcolor{RoyalBlue}{Travel within guides for long distances}}
					\only<3->{\item \textcolor{RoyalBlue}{Can propagate in complex structures}}
					\only<4->{\item \textcolor{RoyalBlue}{High speeds in metals and composites}}
					\only<5->{\item \textcolor{RoyalBlue}{Can be automated using software}}
				\end{itemize}
			\only<6->{\textcolor{OliveGreen}{Lamb waves are a promising global NDE solution for SHM}}
		\end{column}}
	\end{columns}	
\tiny 
(source: https://nojigon.webs.upv.es/index.php)
...
\begin{frame}{Conventional approaches}
	...
	\begin{itemize}
		\item[$\times$] <2-> \textcolor{BrickRed}{Requires a great amount of human labor and computational effort.}
		\item[$\times$] <2->\textcolor{BrickRed}{Demands a high amount of experience of the practitioner.}
		\item[$\times$] <2->\textcolor{BrickRed}{Inefficient with big data which requires a complex computation of damage features.} 
		\end{itemize}
\end{frame}
...
\begin{frame}{Deep learning approach}
	...
	\begin{itemize}
			\item <1->\textcolor{NavyBlue}{Exponential development in computer hardware/software industries.}
			\item <2->\textcolor{NavyBlue}{Machine learning algorithms.}
			\item <3->\textcolor{NavyBlue}{Era of Big data.}
	\end{itemize}	
...
\begin{frame}{Numerical test cases animation of Lamb waves}
		\begin{figure}
			...
			\quad
			\subfloat[\(2^{nd}\) case.]{\animategraphics[autoplay,loop,width=.2\textwidth]{6}{figures/gif_figs/438_convLSTM/438_convLSTM-}{0}{23}}
			\quad
...
\begin{frame}{Experimental results full wavefield based}
		\centering
		\subfloat{\animategraphics[autoplay,loop,width=.245\textwidth]{12}{figures/gif_figs/exp/exp-}{0}{447}} 
		\qquad
		\subfloat{\includegraphics[width=.6\textwidth]{exp_rms_thresholded.png}}
