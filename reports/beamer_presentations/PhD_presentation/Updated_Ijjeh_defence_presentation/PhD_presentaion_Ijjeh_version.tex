...
\note
	...
	\note{
		...
		The delaminations's major and minor axes were randomly selected from interval \([10mm,\ 40mm]\). 
...
%			\quad
		...
		\noindent\textbf{I took the hardest task: 
			\\ 
	...
	\note{
		...
		The kernel also called a filter slides over an input image to extract features.
...
%		GCN addresses the importance of having large kernels at the convolution operations for both localization and classification tasks for semantic segmentation.
	...
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	\note{
		In this slide, I present the autoencoder ConvLSTM model that takes a sequence of consecutive full wavefield frames of guided wave propagation as an input.
		For training purposes, as the delamination location is known, I used a certain number of frames after the initial interaction of guided waves with the damage.
		...
		Then, the output of the decoder is forwarded into the ConvLSTM layer, which is used to handle time series data by learning long-term spatiotemporal features.
