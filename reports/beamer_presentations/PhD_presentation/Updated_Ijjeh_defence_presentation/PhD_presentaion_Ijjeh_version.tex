...
\begin{document}
	...
	\begin{frame}{Objectives}
		\begin{footnotesize}
			\justifying
			\begin{itemize}
				\item \textbf{To develop \textcolor{logoblue}{a novel artificial intelligence (AI) driven diagnostic system} for delamination identification in composite laminates such as carbon fibre reinforced polymers (CFRP).}
		...
		\begin{tcolorbox}			
			\begin{alertblock}{Thesis}
				\justifying
				It is possible to use an end-to-end approach in which a deep neural network (DNN) 
				processes the animation of propagating waves (input) directly into a damage map (output).
			\end{alertblock}
			\begin{alertblock}{Thesis}
				\justifying
				It is possible to speed up the data acquisition process by SLDV by employing an~appropriate deep learning (DL) approach to recover high-resolution (HR) full wavefield scans from a~low-resolution (LR).
			\end{alertblock}
		\end{tcolorbox}		
	\end{frame}
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	\note
	{
		...
		The primary objective of this work is to develop a diagnostic tool based on artificial intelligence for delamination detection in composite laminates.
		...
		Accordingly, I have two theses to confirm:
	}
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	\note
	{
		I can say that the novelty of my work resides in applying for the first time a full wavefield dataset of elastic wave propagation (a numerically generated dataset) as an input to various supervised deep neural networks, which is capable of identifying damage in an end-to-end approach. 
		Additionally, to demonstrate the feasibility of my models, I have compared them with adaptive wavenumber filtering (a conventional signal processing technique for damage imaging).
		
		Furthermore, I have developed a deep learning model to perform an end-to-end superresolution technique regarding elastic wave propagation and their interaction with the damage and the specimen boundaries.
		Also, I compared my model for superresolution image reconstruction with a well-known technique, the compressive sensing theory.		
	}
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	\note
	{
		Here, I present the experimental setup at the laboratory for the SLDV measurements for specimens with artificial damage that will be used to evaluate my developed models.
	...
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	\note{
		The specimens of composite laminates used for experimental evaluation are composed of 16 layers set at the same angle of plain weave fabric with the following characteristics:
	...
	\begin{frame}[t]{Specimens with defects}
		...
		\begin{columns}[T]
			...
			\only<1>{
			\begin{figure}
				\includegraphics[width=0.85\textwidth]{plate_multi_delam_arrangement_large_fonts.png}
			\end{figure}}
			\only<2>{				
				\begin{figure}
					\efbox
					...
					\caption{\footnotesize These specimens were specifically brought to my PhD project.}
				\end{figure}}
			\column{0.5\textwidth}
			\begin{figure}
	...
	{	
		\footnotesize
		In this slide, I the specifications of the specimens used for evaluating the developed deep-learning models are presented.
		
		Specimens with multiple delaminations are shown on the left.
		In which there are three specimen with multiple delamination were located at the same distance, equal to 150 mm from the centre of the plate. placed at different thickness of the plate. 
		
		The specification of the specimen with a single delamination is shown on the right. 
...
%					\item Mindlin-Reissner plate theory
	...
	\begin{frame}{Convolution Neural Networks (CNNs)}
		\begin{itemize}
			\item \alert{Convolutional Neural Network} (CNN) is a powerful feature extraction tool.
	...
	\setcounter{subfigure}{0}
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	\begin{frame}{Conventional machine learning vs. deep learning approaches}
