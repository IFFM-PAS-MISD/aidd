...
\note
	...
	\note{
		...
		The meshes on the left show the modelled delamination in green and the piezoelectric transducer in red for each scenario.
	}
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	\setcounter{subfigure}{0}
	...
	\begin{frame}{Computer vision}
		\begin{columns}[T]
			...
			\begin{column}[c]{0.7\textwidth}
				...
				\only<2>{\begin{figure}
						\centering
						\efbox{\includegraphics[width=1\textwidth]{computer_vision_tasks_2.png}}					
				\end{figure}}											
			\end{column}
		\end{columns}
		\only<2>{\begin{tcolorbox}
		\noindent\textbf{I went with the hardest task: 
			\\ 
			\alert{ Delamination identification (Pixel-wise image segmentation)}}
		\end{tcolorbox}}
	...
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	\note{
		Convolutional Neural Network (CNN) is a powerful deep learning architecture for mainly used with image processing as it can extract complex feature patterns from its inputs using the convolution operation.
		...
		The kernel slides over an input image to extract features.
	...
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	\note{
		In this slide, I present a comparison between conventional machine learning and deep learning approaches utilised for damage detection.
		
		In the conventional approaches, two processes must be performed by the practitioner: 
		The first is to extract the useful features from the registered data then to use a proper classification technique in order to get a prediction.
	...
	\note
	{
		For the purpose of developing and training my deep learning models several data preprocessing were applied as shown in the top table, such as Dataset normalization, data augmentation, cross validation.
		Additionally, to reduce the overfitting problem I have applied some techniques like Early stopping, Hyperparameter tuning and  Regularization.
		
		Furthermore, I have splitted the dataset into three portions as shown in the below table
...
%	\subsection{Developed DL models}
	...
	\note{
		...
		The developed models based on the RMS images as input are:
	...
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	\note{
		In this slide I present the architectures of all my developed deep learning models based on RMS images for delamination identification.
		
		For further details about these models, you can kindly return to my thesis and published papers. 
...
%		GCN addresses the importance of having large kernels at the convolution operations for both localization and classification tasks for semantic segmentation.
	...
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	\note{
		The autoencoder ConvLSTM was trained on a sequence of consecutive full wavefield frames of guided wave propagation.
		
		Now for training the model, as the delamination location is known, I used a certain number of frames after the interaction with the damage.
		...
		Then, the output of the decoder was forwarded into the ConvLSTM layer used to handle time series data by learning long-term spatiotemporal features.
		
		
		For real-life situations where the damage location is not known, the whole frames of the full wavefield is tested thourgh a sliding window that produces an intermediate output at at time.
		Then a root mean squared image representing all intermediate predictions is calculated representing the predicted damage map.		
	}
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%					\caption{LR input \((N_f = 110)\)}
	...
	\note{
		...
		As we can see the DLSR model achieved the highest PSNR and Pearson CC values.
