...
\draw[thick,->,logoblue] (0,0)--(7,0) node[right] {$f$ [Hz]}; % x axis
\draw[thick,->,logoblue] (0,0)--(0,4.5) ; % y axis
\draw[thick,red] (0,0)--(3,3.5)--(6,3.5); % high-pass filter
\draw[thin,dashed,green] (3,3.5)--(3,0); % cut off frequency
\node[logoblue,below] at (3,0) {$f_c$};
\node[logoblue,above] at (2,0.1) {Stopband};
...
\end{alertblock}	
	...
	\draw[thick,->,logoblue] (0,0)--(0,4.5) ; % y axis
	\draw[thick,red] (0,0)--(2,3.5)--(5,3.5)--(7,0); % band-pass filter
	\draw[thin,dashed,green] (2,3.5)--(2,0); % cut off frequency f_L
	\node[logoblue,below] at (2,0) {$f_L$};
	\draw[thin,dashed,green] (5,3.5)--(5,0); % cut off frequency f_H
	\node[logoblue,below] at (5,0) {$f_H$};
	\node[logoblue,above] at (1.1,0.1) {Stopband};
	\node[logoblue,above] at (3.5,0.14) {Passband};
...
\end{frame}
%%%%%%%%%%%%%%%%%%%%
\note{The Lenna picture is one of the most widely used standard test image used for compression algorithms.
It comes from Playboy magazine from 1972.
We will use it as well for some image processing examples by using 2D FFT as it is shown in the animation.
As you remember, 1D FFT in case of signals is a mapping between the time domain and frequency domain.
...
First, we transfom image to the wavenumber domain, then apply a filter mask m hat, and finally going back to the spatial domain by using the inverse Fourier transform.}
