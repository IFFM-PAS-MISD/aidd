...
Finally I will show you a few examples of \textbf{Signal filtering} and \textbf{Image processing}.
...
\(\omega = 2 \pi f\)
\begin{equation*}
S(\textcolor{red}{f}) =  \int \limits_{\textcolor{green}{-\infty}}^{\textcolor{green}{\infty}} s(t) \cos (2 \pi t \textcolor{red}{f})  \ud t \, - \,  i   \int \limits_{\textcolor{green}{-\infty}}^{\textcolor{green}{\infty}}  s(t) \sin(2 \pi t \textcolor{red}{f})  \ud t 
\end{equation*}
...
The index \(k\) denotes sample number and the index \(j\) denotes harmonic number.
...
\(t\) - time\\
\(x\) - space\\
\(\alpha\) - diffusivity of the medium\\
\vspace{5mm}}
\only<5>{Initial condition \(u(x,0)\)\\}
\only<6>{Initial condition \(u(x,0) \xrightarrow {\mathcal{F}} \hat{u}(\kappa,0)\)\\}
\only<5->{
\begin{tikzpicture}
...
\end{equation*}
\begin{equation*}
u_x \xrightarrow{\mathcal{F}} i \kappa \hat{u} (\kappa, t)
\end{equation*}
\begin{equation*}
u_{xx} \xrightarrow{\mathcal{F}}  - \kappa^2 \hat{u}(\kappa , t)
\end{equation*}
\begin{equation*}
u_{t} \xrightarrow{\mathcal{F}} \hat{u}_t = \frac{\partial \hat{u}(\kappa,t)}{\partial t}
\end{equation*}}
\only<3->{
\vspace{-3mm}
\begin{equation*}
...
\end{frame}
%%%%%%%%%%%%%%%%%%%%
\note{{\scriptsize Heat conduction equation has a form of a partial differential equation which can be written as derivative of temperature field over time is equal to alpha square times second derivative of temperature field over x.
So, u is the temperature, t is the time, x is space and alpha is the diffusivity of the medium.
It turns out that, the Fourier transform can be used to approximate partial differentials.
You already know how to apply one-dimensional Fourier transform to signals.
Now, we will be using Fourier transform for mapping spatial domain x into wavenumber domain denoted here by kappa.
We do not apply Fourier transform to the time domain.
Fourier transform has very nice property which allows to use it for approximation of partial derivatives.
So it is easy to compute derivatives in space in this case just by Fourier transforming and multiplying by i kappa or minus kappa square for the first and second derivative, respectively.
In case of the left hand side of the partial differential equation, we transform u from spatial domain to frequency domain but the differentiation in respect to time is applied afterwards.
Essentially, we applied the Fourier transform to the whole partial differential equation.
As you can see now we have only differentiation in respect to time so this is the system of ordinary differential equations (ODE) which is easy to solve.
Actually, it is n decoupled ODEs, one for each kappa j. This system of ODEs can be written in a matrix form.
Here, kappa 1 to n are just a numbers.
}
...
\newpage
\note{In order to have well-posed problem we need to add initial conditions at the time equal zero.
Let's assume that our initial conditions can be represented by the top-hat function.
In order to better visualize it, you can think of three rods: the rod in the middle is hot and the rods at the left hand side and right hand side are cold. 
We would like to know how the tempereture evolve with time immediately after the rods  come into contact.
Because we are solving this problem in wavenumber domain, we also have to transform function of intial condition from the space domain to the time domain.
Once, the system of ODEs is solved in the wavenumber domain, we will apply inverse Fourier transform in order to obtain spatial temperature distribution.
OK, Let's try to code it in Matlab.
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Digital filter purposes:
\begin{enumerate}
	\item separation of signals that have been combined
	\item restoration of signals that have been distorted in some way. 
\end{enumerate}
\end{frame}
%%%%%%%%%%%%%%%%%%%%
\note{Digital filters are used for two general purposes: (1) separation of signals that have been combined, and (2) restoration of signals that have been distorted in some way. 
Signal separation is needed when a signal has been contaminated with interference, noise, or other signals. 
For example, imagine a device for measuring the electrical activity of a baby's heart while still in the womb. 
...
Analog (electronic) filters can be used for the same tasks; 
...
In fact, their extraordinary performance is one of the key reasons that digital signal processing has become so popular. 

I will introduce you the most popular digital filters.}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Signal filtering(1)}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{alertblock}{Low-pass filter}
...
\end{alertblock}
\begin{tikzpicture}
\draw[thick,->,logoblue] (0,0)--(7,0) node[right] {$f$ [Hz]}; % x axis
\draw[thick,->,logoblue] (0,0)--(0,4.5) ; % y axis
\draw[thick,red] (0,3.5)--(3,3.5)--(4.5,0)--(6,0); % low-pass filter
\draw[thin,dashed,green] (3,3.5)--(3,0); % cut off frequency
\node[logoblue,below] at (3,0) {$f_c$};
\node[logoblue,above] at (2,0.14) {Passband};
\node[logoblue,above] at (5.5,0.1) {Stopband};
\end{tikzpicture}
\end{frame}
%%%%%%%%%%%%%%%%%%%%
\note{A low-pass filter (LPF) is a filter that passes signals with a frequency lower than a selected cut-off frequency and attenuates signals with frequencies higher than the cut-off frequency.}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Signal filtering(2)}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{alertblock}{High-pass filter}
A high-pass filter (HPF) is a filter that passes signals with a frequency higher than a certain cut-off frequency and attenuates signals with frequencies lower than the cut-off frequency.
\end{alertblock}
\begin{tikzpicture}
\draw[thick,->,logoblue] (0,0)--(7,0) node[right] {$f$ [Hz]}; % x axis
\draw[thick,->,logoblue] (0,0)--(0,4.5) ; % y axis
\draw[thick,red] (0,0)--(2.5,0)--(3,3.5)--(6,3.5); % high-pass filter
\draw[thin,dashed,green] (3,3.5)--(3,0); % cut off frequency
\node[logoblue,below] at (3,0) {$f_c$};
\node[logoblue,above] at (1.5,0.1) {Stopband};
\node[logoblue,above] at (4,0.14) {Passband};
\end{tikzpicture}	
\end{frame}
%%%%%%%%%%%%%%%%%%%%
\note{Most discussions on filter design only give examples of low-pass filters. 
It is because low-pass filter can be converted to high-pass filter.
There are two methods for the low-pass to high-pass conversion: spectral inversion and spectral reversal. Both are equally useful.
...
\end{alertblock}	
	\begin{tikzpicture}
	\draw[thick,->,logoblue] (0,0)--(9,0) node[right] {$f$ [Hz]}; % x axis
	\draw[thick,->,logoblue] (0,0)--(0,4.5) ; % y axis
	\draw[thick,red] (0,0)--(1.5,0)--(2,3.5)--(4,3.5)--(4.5,0)--(7,0); % band-pass filter
	\draw[thin,dashed,green] (2,3.5)--(2,0); % cut off frequency f_L
	\node[logoblue,below] at (2,0) {$f_L$};
	\draw[thin,dashed,green] (4,3.5)--(4,0); % cut off frequency f_H
	\node[logoblue,below] at (4,0) {$f_H$};
	\node[logoblue,above] at (0.8,0.1) {Stopband};
	\node[logoblue,above] at (3,0.14) {Passband};
	\node[logoblue,above] at (5.8,0.1) {Stopband};
	\end{tikzpicture}
\end{frame}
%%%%%%%%%%%%%%%%%%%%
\note{Band-pass filter can be obtained by convolving low-pass and high-pass filters.}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\end{frame}
%%%%%%%%%%%%%%%%%%%%
\note{Image processing can have various purposes such as image enhancement, object detection, image segmentation, etc.
We often want to enhance an image in certain way.
Image enhancement is the process of adjusting digital images so that the results are more suitable for display or further processing.
It can be use for example for noise removal, debluring or filtering.}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{columns}[T]
	\begin{column}{0.4\textwidth}
		\begin{figure}
			\animategraphics[controls,autoplay,loop,width=0.8\textwidth]{3}{Lena}{1}{46}
			%\includegraphics[width=0.8\textwidth]{Lena1.png}
			\caption{Lenna}
		\end{figure}
	\end{column}
...
\end{frame}
%%%%%%%%%%%%%%%%%%%%
\note{The Lenna picture is one of the most widely used standard test image in the field of compression algorithms.
It is a picture of the Swedish model Lena Forsen which comes from Playboy magazine from 1972.
We will use it as well for some image processing examples by using 2D FFT as it is shown in the animation.
{\scriptsize There is a story behind this image.
Alexander Sawchuk, assistant professor at the University of Southern California Signal and Image Processing Institute along with graduate student was searching the lab for a good image to scan for a conference paper. 
They wanted to have something more distinctive from boring test photos. 
They were looking for a photo with a human face with a good dynamic range.
Then, somebody just came in with a recent issue of Playboy. 
Engineers took part of that photo and scanned it which resulted in the 512 by 512 pixel image.
This scan became one of the most used images in computer history.
But let's go back to the main topic.}
}
\newpage
\note{As you remember, 1D FFT in case of signals is a mapping between the time domain and frequency domain.
2D FFT on the other hand is a mapping between 2D spatial domain (x,y) and 2D wavenumber domain.
The strategy is the same as in case of signal filtering.
First, we transfom image to the wavenumber domain. Image here is denoted by the captial I. Then we apply a filter mask m hat, and finally going back to the spatial domain by using the inverse Fourier transform.
It should be noted that element-wise multiplication is performed in the wavenumber domain.
Such operation is performed in Matlab by using dot asterix symbol.
Let's move to Matlab so you can code such animations yourself.
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Assignment}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{enumerate}	
	\item Write a code in Matlab or other preferred language like Python or Julia which plots a function:
	\begin{equation*}
...
\end{frame}
%%%%%%%%%%%%%%%%%%%%
\note{I have prepared two tasks for you. 
In the first task, I want you to write a code in Matlab  or other preferred language like Python or Julia.
But I think that it will be easiest to use Matlab scripts which are attached to this lecture as a starting point.
The code should plot a function defined here. It should perform Fourier transform of this function and plot its magnitude.
As you can see there are two sets of frequency parameters for definition of this function.
I want you to write a few sentences describing what you see in the time domain and frequency domain for each frequency pair.

In the second task, I want you to write an answer to the following problem.
Let's suppose, that we are using a software that provides a results in a form of a function sampled at Gauss-Lobatto-Legendre points which are non-equidistant (like here).
Our task is to perform the Fourier transform of that data. 
So we have an arbitrary function sampled at these points.
What will be the strategy of performing the Fourier transform on sampling points which are non-equidistant?
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
I am aware that the presentation was given in a quite condensed form.
Therefore, I expect that you might not grasp everything.
