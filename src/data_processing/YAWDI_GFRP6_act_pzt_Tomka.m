...
% Laminat_6_GFRP_Tomka, 2mm thick, excitation by ACT and PZT
...
%%
specimen_name='GFRP6_act_pzt';
% full field measurements
list = {'chirp_interp','pzt40_packet_interp','act40_left_interp','act40_interp'};
freq_list =[40,40,40]; % frequency list in kHz according to files above (max 4 frequencies)
...
fband = 30e3; % frequency band width [Hz]
...
selected_frames={40:140,40:140,40:140,40:140}; % selected frames for Hilbert transform
...
raw_data_path = ['/pkudela_odroid_laser/GFRP_Tomka/',specimen_name,'/'];
...
%                             Thickness_map(i,j) = thickness(J);
                ...
                hold on;
                if(damage_outline) 
                    plot3(delam1(:,1),delam1(:,2),repmat(4,[length(delam1),1]),'k:','LineWidth',0.5); 
                    plot3(delam2(:,1),delam2(:,2),repmat(4,[length(delam2),1]),'k:','LineWidth',0.5); 
                    plot3(delam3(:,1),delam3(:,2),repmat(4,[length(delam3),1]),'k:','LineWidth',0.5); 
                    plot3(delam4(:,1),delam4(:,2),repmat(4,[length(delam4),1]),'k:','LineWidth',0.5); 
                end
                %caxis([0 5]);   
                caxis([Smin 3]); 
                %caxis([0 4.5e-3]); 
