function [FilterMask,RMSF,ERMSF,WRMSF] = AdaptiveReverseFilteringMask(Data,time,WL,mask_thr,PLT)
...
if PLT > 1
    ...
    hold on
    line([threshold threshold], [0 (100-threshold)/100],'LineWidth',2,'LineStyle','--','Color',[0.2 .2 1]);
    line([-max(CDF_x)*0.01 threshold], [(100-threshold)/100 (100-threshold)/100],'LineWidth',2,'LineStyle','--','Color',[0.2 .2 1])
    drawnow;
end
...
% Reverse filter mask
FilterMask = -1*(FilterMask - 1);
for i=1:round(cols)
    for j=1:round(rows)
        if(round(sqrt((i-257)^2+(j-257)^2))>20)
            FilterMask(i,j) = 0;
        end
    end
end
