...
########################################################################################################################
def layer_bb(input,i):
    layer1 = Conv2D(i*8,(1,1),padding='same')(input)
    layer1 = Activation('relu')(layer1)
    layer2 =Conv2D(i*8,(3,3),padding='same')(layer1)
    layer2 = Activation('relu')(layer2)
    layer3 = Conv2D(i*8,(1,1),padding='same')(layer2)
    layer3 = Activation('relu')(layer3)
    return  keras.layers.Concatenate()([input,layer3])

# i: number of layers per block, n : factor multiplied with filter size, s: whether last block or not
def block(input,i,n,s):
    layerx = input
    if s == 0:
        for j in range (i):
            layerx = layer_bb(input,n)
            input = layerx
    else:
        layerx = Conv2D(64, (3, 3),dilation_rate = (2,2), padding='same')(input)
