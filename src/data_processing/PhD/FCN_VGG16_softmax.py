...
#####################################
u1 = keras.layers.UpSampling2D((2, 2),interpolation='bilinear')(d5)  # ,interpolation='bilinear'
#####################################
...
#####################################
u2 = keras.layers.UpSampling2D((2, 2),interpolation='bilinear')(conv_16)
#####################################
...
#####################################
u3 = keras.layers.UpSampling2D((2, 2),interpolation='bilinear')(conv_19)
#####################################
...
#####################################
u4 = keras.layers.UpSampling2D((2, 2),interpolation='bilinear')(conv_22)
#####################################
...
#####################################
u5 = keras.layers.UpSampling2D((2, 2),interpolation='bilinear')(conv_24)
#####################################
...
###################
model.compile(optimizer='adam', loss=keras.losses.categorical_crossentropy, metrics=[iou_metric])#[keras.metrics.binary_accuracy])
model.fit(np.asarray(x_train), np.asarray(y_train), batch_size=2, epochs=10, shuffle=True, validation_split=0.2)
model.save('VGG16_softmax.h5')
gc.collect()
print('model saved')
############################################
