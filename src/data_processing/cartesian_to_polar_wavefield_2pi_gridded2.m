function [Data_polar,number_of_points,radius] = cartesian_to_polar_wavefield_2pi_gridded2(Data,Lx,Ly,beta)
...
% 
% Syntax: [Data_polar,number_of_points,radius] = cartesian_to_polar_wavefield_2pi_gridded(Data,Lx,Ly,beta) 
% 
% Inputs: 
...
% 
% Example: 
%    [Data_polar,number_of_points,radius] = cartesian_to_polar_wavefield_2pi_gridded(Data,Lx,Ly,beta) 
%    [Data_polar,number_of_points,radius] = cartesian_to_polar_wavefield_2pi_gridded(Data,Lx,Ly,[0:15:360])  
% 
...
number_of_points=round(sqrt(nPointsx*nPointsx+nPointsy*nPointsy)); % # no of grid points for R coordinate
...
Data_polar=zeros(number_of_angles,number_of_points,number_of_time_steps);
 %%
[XI,YI] = meshgrid(linspace(lxmin,lxmax,nPointsx),linspace(lymin,lymax,nPointsy)); % 
% X=reshape(XI,[],1);
% Y=reshape(YI,[],1);
 %% 
radius=sqrt(lxmax*lxmax+lymax*lymax);
...
F = griddedInterpolant(XI',YI',ZI','linear','none'); % requires ndgrid format; no extrapolation
...
% ================ [cartesian_to_polar_wavefield_2pi_gridded.m] ================  
