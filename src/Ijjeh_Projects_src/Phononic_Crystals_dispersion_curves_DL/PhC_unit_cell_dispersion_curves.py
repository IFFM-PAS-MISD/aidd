...
def build_model(HP):
    ...
    x = inputs_1
    # skip_tensor = []
    for i in range(HP.Int('conv_blocks', min_value=3, max_value=8, step=1)):  # Int specifies the dtype of the values
        # filters = HP.Int('filters_' + str(i), min_value=8, max_value=32, step=4)
        k_size = HP.Int('k_size', min_value=3, max_value=6, step=2)
        x = conv_block(x, hyperparameters['num_filters'] * 2 ** i, k_size)
...
tuner = kt.BayesianOptimization(
    ...
    project_name='PhC_dispersion_diagrams',
    overwrite=True)

# calling dataset func
X, Y, s = load_dataset()

checkpoint_filepath = env_path + 'temp/checkpoint/PC_model_HPO_%s_%d_lr_%s_filters_%d_levels_%d_batches_%d_epochs_%d_dropout_%s_val_split_%s_hidden_%d.h5' % (
    s,
    hyperparameters['samples'],
    hyperparameters['learning_rate'],
...
# f'filters_{i}' for i in range(n_blocks)] +
for hp in [f'pooling_{i}' for i in range(n_blocks)] + ['h_levels'] \
          + ['units'] + ['units1'] + ['dropout'] + ['learning_rate'] + ['k_size']:
