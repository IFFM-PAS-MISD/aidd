...
def load_dataset():
    ...
    arr_pred = np.load('LR_damage_prediction_num.npy')
    ...
    # arr_pred = arr_pred.reshape((512, 32, 32, 1))
    return arr_pred
...
def testing():
    ...
    for output_case in range(1, 95):
        ##################################################################
        # for Labels
        path_GT_ = path_GT + '%d_output/' % (output_case + 380)
        ##################################################################

        path_ = path + 'Output_img_%d' % (output_case + 380)
        try:
            os.mkdir(path_)
        except OSError:
        ...
        xtest = x_test[(output_case - 1) * 512:(output_case - 1) * 512 + 512]
        print(xtest.shape)
        prediction = model.predict(xtest, batch_size=1)
        prediction = np.asarray(prediction)
        print(prediction.shape)
        # for each test sample there are 512 predicted frames
        for i in range(512):
            ############################################################################################################
            os.chdir(path_GT_)
            ...
            print(SR_pred.shape)
            ############################################################################################################
            plt.figure(figsize=(4, 2), dpi=512)
