...
def load_dataset():
    ...
    delam_GT_img = delam_GT_img.reshape((475, 512, 32, 32))
    ...
    Train_x, Val_x_samples, Train_label, Val_y_samples = train_test_split(X_train, Y_train,
                                                                          ...
                                                                          shuffle=False,
                                                                          random_state=1988)
    health_siganl = x_lr_frame[0]
    damaged_signal = Train_label[0]
    diff = (health_siganl - damaged_signal)
    for i in range(512):
        plt.imshow(diff[i])
        plt.show()
...
test_x, test_label = load_dataset()

test_x_1 = test_x[:, 0:1024]
test_x_2 = test_x[:, 1024:]
...
print(model_name)
model = load_model(model_name, compile=False)  # , compile=False
model.summary()

prediction = model.predict([test_x_1, test_x_2], batch_size=params.get('batches'))
print(prediction.shape)
os.chdir(env_path + 'num/')
np.save('Encoder_ANN_coords_prediction_num', prediction)
