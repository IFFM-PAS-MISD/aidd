...
delam_coords = delam_coords[params['n_size']: params['n_size'] + 64]
...
delam_GT_img = np.invert(delam_GT_img[params['n_size']:params['n_size'] + 64]) - 254
...
def load_dataset():
    ...
    lr_inputs = x_lr_frame[params['n_size']:params['n_size'] + 64]
    lr_labels = y_lr_frame[params['n_size']:params['n_size'] + 64]
...
model_name = 'SA_ConvLSTM_%s_time_stamps_%s_%s_%d_lr_filters_%d_levels_%d_batches_%d_epochs_%d_dropout_%s_val_split_%s_hidden_%d.h5' % (
    params['shape'],
    params['time_stamps'],
    params['samples'],
...
model = load_model(model_name, custom_objects={'SaConvLSTM2D': SaConvLSTM2D})  # , compile=False
model.summary()

prediction = model.predict([X1, delam_GT_img], batch_size=params['batches'])
print(prediction.shape)
os.chdir(env_path + 'num/')
np.save('ConvLSTM_model_coords_prediction_num', prediction)
